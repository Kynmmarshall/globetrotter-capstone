...
single GET request, same lightweight-HTTP-over-SDK approach as ai.py and
stats.py, instead of adding the google-auth package as a dependency.
Google's endpoint already validates the token's signature, issuer and
...
logger = logging.getLogger("trip_io.google_auth")
